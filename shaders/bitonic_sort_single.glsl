...
void sort_1_to_1024(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_1024_to_2048(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_2048_to_4096(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_4096_to_8192(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_8192_to_16384(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_16384_to_32768(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_16384_to_32768_1(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_16384_to_32768_2(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_32768_to_65536(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_32768_to_65536_1(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_32768_to_65536_2(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_65536_to_131072_1(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_65536_to_131072_2(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_131072_to_262144_1(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void sort_131072_to_262144_2(uint pass) {
    uint lindex = gl_LocalInvocationIndex;
    uint gid = 1024 * pass + lindex;
...
void main() {
    ...
    for (uint i = 0; i < log2(u_max_count); i++) {
        uint mask1 = (2 << i) - 1;
        uint mask2 = 1 << i;

        for (uint k = 0; k < u_max_count / 1024; k += 1) {
            uint gid1 = k*1024 + lidx;
            uint gid2 = gid1 ^ mask1;

            if (ping) {
                uint x1 = b_values_in[gid1];
                uint x2 = b_values_in[gid2];
                b_values_out[gid1] = compare_and_select(x1, x2, (gid1 & mask2) != 0);
            } else {
                uint x1 = b_values_out[gid1];
                uint x2 = b_values_out[gid2];
                b_values_in[gid1] = compare_and_select(x1, x2, (gid1 & mask2) != 0);
            }
        }
        ping = !ping;
        groupMemoryBarrier();
    ...
    */
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_1_to_1024(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_1024_to_2048(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_2048_to_4096(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_4096_to_8192(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_8192_to_16384(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_16384_to_32768_1(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_16384_to_32768_2(i);
    for (uint i = 0; i < u_max_count / 1024; i += 1) sort_32768_to_65536_1(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_32768_to_65536_2(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_65536_to_131072_1(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_65536_to_131072_2(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_131072_to_262144_1(i);
    //for (uint i = 0; i < u_max_count / 1024; i += 1) sort_131072_to_262144_2(i);

    //if (u_max_count > 0*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_1_to_1024(i); groupMemoryBarrier(); };
    //if (u_max_count > 1*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_1024_to_2048(i); groupMemoryBarrier(); };
    //if (u_max_count > 2*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_2048_to_4096(i); groupMemoryBarrier(); };
    //if (u_max_count > 4*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_4096_to_8192(i); groupMemoryBarrier(); };
    //if (u_max_count > 8*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_8192_to_16384(i); groupMemoryBarrier(); };
    //if (u_max_count > 16*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_16384_to_32768_1(i); groupMemoryBarrier(); };
    //if (u_max_count > 16*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_16384_to_32768_2(i); groupMemoryBarrier(); };
    //if (u_max_count > 32*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_32768_to_65536_1(i); groupMemoryBarrier(); };
    //if (u_max_count > 32*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_32768_to_65536_2(i); groupMemoryBarrier(); };
    //if (u_max_count > 64*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_65536_to_131072_1(i); groupMemoryBarrier(); };
    //if (u_max_count > 64*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_65536_to_131072_2(i); groupMemoryBarrier(); };
    //if (u_max_count > 128*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_131072_to_262144_1(i); groupMemoryBarrier(); };
    //if (u_max_count > 128*1024) {for (uint i = 0; i < u_max_count / 1024; i += 1) sort_131072_to_262144_2(i); groupMemoryBarrier(); };
}