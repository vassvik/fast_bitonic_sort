...
void sort_4096_to_8192() {
    ...
    for (uint i = 0; i < 8; i++) {
        uint idx = gid^(i*512);
        sorted[i] = b_values_in[idx];
        sorted[i+8] = b_values_in[idx^8191];
    }

    for (uint i = 0; i < 8; i++) sorted[i] = compare_and_select(sorted[i], sorted[i+8], (gid&4096) != 0);
    for (uint i = 0; i < 4; i++) sorted[i] = compare_and_select(sorted[i], sorted[i+4], (gid&2048) != 0);
    for (uint i = 0; i < 2; i++) sorted[i] = compare_and_select(sorted[i], sorted[i+2], (gid&1024) != 0);
    for (uint i = 0; i < 1; i++) sorted[i] = compare_and_select(sorted[i], sorted[i+1], (gid&512) != 0);
...
void sort_8192_to_16384() {
    ...
    for (uint i = 0; i < 16; i++) {
        uint idx = gid^(i*512);
        sorted[i] = b_values_in[idx];
        sorted[i+16] = b_values_in[idx^16383];
    }

    for (uint i = 0; i < 16;  i++) sorted[i] = compare_and_select(sorted[i], sorted[i+16], (gid&8192) != 0);
    for (uint i = 0; i < 8;  i++) sorted[i] = compare_and_select(sorted[i], sorted[i+8], (gid&4096) != 0);
    for (uint i = 0; i < 4;  i++) sorted[i] = compare_and_select(sorted[i], sorted[i+4], (gid&2048) != 0);
    for (uint i = 0; i < 2;  i++) sorted[i] = compare_and_select(sorted[i], sorted[i+2], (gid&1024) != 0);
    for (uint i = 0; i < 1;  i++) sorted[i] = compare_and_select(sorted[i], sorted[i+1], (gid&512) != 0);
...
void sort_32768_to_65536_1() {
    ...
    T sorted[8];
    for (uint i = 0; i < 4; i++) sorted[i] = b_values_in[gid^(i*8192)];
    for (uint i = 0; i < 4; i++) sorted[4+i] = b_values_in[gid^(i*8192)^65535];
    ...
    b_values_out[gid] = sorted[0];
    b_values_out[gid^8192] = sorted[1];
} 
...
void sort_32768_to_65536_2() {
    ...
    for (uint i = 0; i < 8; i++) {
        uint idx = gid^(i*512);
        sorted[i] = b_values_in[idx];
        sorted[8+i] = b_values_in[idx^4096];
    }

    for (uint i = 0; i < 8; i++) sorted[i] = compare_and_select(sorted[i], sorted[i+8],  (gid&4096) != 0);  // 8192
    for (uint i = 0; i < 4; i++) sorted[i] = compare_and_select(sorted[i], sorted[i+4],  (gid&2048) != 0);  // 8192
    for (uint i = 0; i < 2; i++) sorted[i] = compare_and_select(sorted[i], sorted[i+2],  (gid&1024) != 0);  // 4096
    for (uint i = 0; i < 1; i++) sorted[i] = compare_and_select(sorted[i], sorted[i+1],  (gid&512) != 0);  // 2048
